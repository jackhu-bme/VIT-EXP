...
save_mask_selected_dir = "./train_mask_final_new_test"
...
def select_compare_save_single(train_img_path, save_mask_selected_dir):
        ...
        if mask_data.shape[-3:] != img_data.shape[-3:]:
            ...
            shape_error_message = f"mask shape {mask_data.shape} is not the same as img shape {img_data.shape}"
        ...
        return (0, train_img_path, shape_error_message)
...
if __name__ == "__main__":
    ...
    #         f.write(f"{error}\n")
    # collect the shape error cases
    shape_error_list = [res for res in res_list if res[2] is not None]
    # write to file
    with open("shape_error_list_mask_full.txt", "w") as f:
        for error in shape_error_list:
