...
class CTCLIP(nn.Module):
    ...
    def create_fusion_head(self, config):
        type_name = config.get("type", None)
        if type_name == "mlp":
            mlp_config = config.get("mlp", {})
            mlp_kwargs = dict(
                head_n_layers = mlp_config.get("n_layers", 2),
                head_layer_type = mlp_config.get("layer_type", "mlp"),
                head_in_dim = mlp_config.get("in_dim", 16),
                head_mid_dim = mlp_config.get("mid_dim", 16),
                head_out_dim = mlp_config.get("out_dim", 1)
            )
            return self.create_head(**mlp_kwargs)
        elif type_name is None:
            return None
        else:
            raise ValueError(f"Unsupported fusion head type: {type_name}")
    ...
    def open_seg_loss(self, seg_preds, seg_mask_flatten, prompt_logits_batch, return_class_loss=False):
        ...
        # exit()
        choose_cls = self.open_seg_loss_hyper_config.get("choose_cls", None)
        if choose_cls is not None:
            # choose the class by indexes in choose_cls
            choose_cls = [int(cls_ind) for cls_ind in choose_cls]
        ...
        if self.open_seg_loss_type == "cos_sim_l2":
            ...
            if return_class_loss:
                ...
                open_seg_loss = self.bce_no_reduction_criterion(sim_all, seg_mask_flatten)
            ...
            # do not use the similarity, use the fusion head to get the logits
            assert self.fusion_head is not None
            # seg_preds: [B, L, n_hidden_dim=8]
            # seg_mask_flatten: [B, L, C]
            # prompt_logits_batch: [B, C, n_hidden_dim=8]
            n_hidden_dim_preds = seg_preds.shape[-1]
            n_hidden_dim_prompt = prompt_logits_batch.shape[-1]
            # concat after broadcasting
            concat_logits_for_fusion = torch.cat([seg_preds.unsqueeze(2).expand(-1, -1, C, -1), prompt_logits_batch.unsqueeze(1).expand(-1, L, -1, -1)], dim=-1)
            # print(f"concat_logits_for_fusion shape: {concat_logits_for_fusion.shape}") guess: (B, L, C, 32)
            concat_logits_for_fusion = concat_logits_for_fusion.reshape(-1, n_hidden_dim_preds + n_hidden_dim_prompt)
            # fusion to 1 logit
            fusion_logits = self.fusion_head(concat_logits_for_fusion)
            # print(f"fusion_logits shape: {fusion_logits.shape}") # guess: (B*L*C, 1)

            # use the sigmoid to get the probability
            p = self.sigmoid(fusion_logits).reshape(-1, C)
    ...
    def forward_batch_image_open_seg(self, batch, device=None, accelerator=None, **kwargs):
        ...
        open_seg_loss, class_seg_loss = self.open_seg_loss(seg_preds, seg_mask_flatten, prompt_logits_batch, return_class_loss=True) # keep the start index same
            ...
            with torch.no_grad():
                ...
                for i in range(C_seg):
                    ...
                    if self.fusion_head is not None:
                        # use the fusion head to process the logits
                        sim = self.fusion_head(torch.cat([seg_preds, prompt_logits.unsqueeze(1).expand(-1, D_down*W_down*H_down, -1)], dim=-1))
